...
PINACT_VERSION="${VERSION:-"latest"}"

REPOSLUG="suzuki-shunsuke/pinact"
...
echo "installing feature pinact..."
...
URL="${RELEASE_URL}${PINACT_VERSION}/pinact_linux_${ARCH}.tar.gz"
echo "from: ${URL}"

curl -sSL -o /tmp/pinact.tar.gz "${URL}"
ls -l /tmp/pinact.tar.gz
tar xzof /tmp/pinact.tar.gz -C /usr/local/bin/ pinact
