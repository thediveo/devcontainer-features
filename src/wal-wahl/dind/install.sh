...
if type docker > /dev/null 2>&1 && type dockerd > /dev/null 2>&1; then
    ...
    if [ "${USE_MOBY}" = "true" ]; then
        ...
        # Install compose
        ##DISABLED## apt-get -y install --no-install-recommends moby-compose || err "Package moby-compose (Docker Compose v2) not available for OS ${ID} ${VERSION_CODENAME} (${architecture}). Skipping."
    else
